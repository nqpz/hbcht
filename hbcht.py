...
c_template = b'''
// Generated by hbcht <http://metanohi.name/projects/hbcht/>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void hbcht_inc_cell(HBCHTList *cells, int pos, inc) {

}

void hbcht_dec_cell(HBCHTList *cells, int pos, inc) {
    hbcht_inc_cell(cells, pos, -inc);
}

int hbcht_get_cell_value(HBCHTList *cells, int pos) {
...
c_template_mainfunc = b'''
int main (int argc, char* argv[]) {
    IntList *il = hbcht_text_to_ints(argv, argc);
...
class CarProgram:
    ...
    def _get_gotos(commands):
        return [sorted(a for x, a in filter(lambda kv: kv[0] in
                                            (GOTO, IF), commands))]
    ...
    def _python_compile(f, funconly, commands, begs, inptext, outtext):
        ...
        for x, a in commands:
            ...
            if x == DECREMENT:
                ...
                code += d + 'break\n'
                if not j + 1 in begs and j != len(commands) - 1:
                    code += dd + 'elif j == {}:\n'.format(j + 1)
            j += 1
